		...
		$('#JoinUsNow').on('shown.bs.modal', function () {
			...
			$('#login_form').on('submit', function(e) {
				...
				$.post("https://api.democratech.co/v2/auth/login",{'email':email})
					.done(function(data){ window.location.href= data['redirect_url']; })
					.error(function(data) {
						console.log(data);
						$('#input_pb').show();
						$('#input_loading').hide();
	...
	if ($('#fundraising_bar')!=null) {
		...
		if (app_stats==null) {
			$.get("https://api.democratech.co/v1/app/stats", function( data ){
				...
				$('#nb_citoyens_app').html("<span class='nb_citizens_fig'>"+nb_citizens+"</span> citoyen(ne)s participent... Et vous ?");
